...
func (c *Client) EnumerateCiphers(hostname, ip, port string, options clients.ConnectOptions) ([]string, error) {
	...
	for _, v := range toEnumerate {
		...
		// Mirror the zero-timeout guard at lines 118-122: when Timeout is unset (0),
		// use a plain Background context (no expiry) rather than an already-expired one.
...
// which evaluates Handshake() synchronously before the send, making the
// ctx.Done() branch unreachable — the select could never cancel a stuck
// handshake (#819).
//
